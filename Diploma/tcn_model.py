...
logger = getLogger('warnings_logger')
...
class TCNModel():
    ...
    def __init__(self,
        ...
        if kernel_size > input_chunk_length:
            logger.warning("The kernel size must be strictly smaller than the input length.")
            raise ValueError("The kernel size must be strictly smaller than the input length.")
    ...
    def output_params(self, train_sample: torch.Tensor):
        input_dim = train_sample.shape[-1]
        return input_dim

    def _create_model(self, train_sample: torch.Tensor) -> torch.nn.Module:
        # samples are made of (past_target, past_covariates, future_target)
        input_dim = train_sample.shape[-1] #+ (train_sample[1].shape[1] if train_sample[1] is not None else 0)
        output_dim = train_sample.shape[-1]
